	...
	<div class="inventory flex flex-wrap p-4 mx-auto lg:mt-24">
		...
		<div class="seeds w-auto" style="width: 33.3333%;">	
			<div class="rounded-md bg-gray-100 pt-4 m-2 overflow-hidden">
				...
				<ul class="">
					<li v-for="plantType in listOfType(seedsData.seeds)" :key="plantType.id"
					...
					:title="plantType.notes">
						...
						class="bg-gray-100">
							<li v-for="seedsPack in seedsOfType" :key="seedsPack.id"
							...
							class="">
								...
								:class="{ 'bg-gray-300 hover:bg-gray-300 hover:text-gray-900': selectedSeedPack === seedsPack.id }">
									<h4 class=" font-mono text-sm">{{ seedsPack.name }}</h4>								
									
										
										<div class="items-center px-1 py-0.5 rounded bg-gray-50">Всего семян:{{ seedsPack.numberOfSeeds * seedsPack.amount }}</div>
		...
		<div class="seedlings w-auto" style="width: 33.3333%;">
			<div class="rounded-md bg-gray-100 pt-4 m-2 overflow-hidden">
				...
				<ul>
					<li v-for="plantType in listOfType(seedlingData.seedlings)" :key="plantType.id"
					...
					:title="plantType.notes">
						...
						class="bg-gray-100">
							<li v-for="seedlings in seedlingsOfType" :key="seedlings.id"
							@click.stop="seedlingItemSelect(seedlings.id)"
							class="">
								<div class="flex justify-between items-center px-3 py-2 text-gray-700 border-gray-400 cursor-pointer hover:bg-gray-200 hover:text-gray-900"
								:class="{ 'bg-gray-300 hover:bg-gray-300 hover:text-gray-900': selectedSeedlingItem === seedlings.id }">
									<h4 class=" font-mono text-sm">{{ seedlings.name }}</h4>								
									<div class="items-center px-1 py-0.5 rounded bg-gray-50">Всего саженцев:{{  seedlings.amount }}</div>
								</div>
  ...
  components: {
	...
	data() {
		return {
			...
			seedlingData: {
				seedlings: [
					{'id': 1, 'name': 'corn', 'typePlant': 1, 'amount': 2, 'price': 100, 'manufacturerId': 1, 'purchaseDate': '2020.03.11', 'plantDate': '2020.03.12'},
					{'id': 2, 'name': 'cornotca', 'typePlant': 1, 'amount': 5, 'price': 100, 'manufacturerId': 3, 'purchaseDate': '2020.03.11', 'plantDate': '2020.03.12'},
					{'id': 3, 'name': 'cyber cucumber', 'typePlant': 2, 'amount': 3, 'price': 100, 'manufacturerId': 2, 'purchaseDate': '2020.03.11', 'plantDate': '2020.03.12'}
				],
			},
	...
	methods: {
		...
		changingSeedling(item) {
			let seedlingInd = this.seedlingData.seedlings.findIndex( seedlings => seedlings.id === item.id);			
			if(seedlingInd != -1) {
				if(item.amount <= 0) {
					if(this.response) {
	...
	computed: {
		...
		seedlingsOfType() {
			return this.seedlingData.seedlings.filter( item => item.typePlant === this.selectedPlantOfSeedlings);
		},
	},
}
</script>

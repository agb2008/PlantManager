  ...
  <div class="my-container">
    <main class="main" v-if="isDataGotten">
      <div class="garden-bed-panel">
        <div
          class="icon-doing"
          v-for="(item, k) of doingsArray"
          :key="k"
          v-on:click="chooseIconDoing(item, $event)"
        >
      ...
      <div class="garden-bed-table flex flex-col">
        <div class="tabel-row flex" v-for="r of quantityRows" :key="r">
          <div
            class="table-cell"
            v-for="c of quantityColumns"
            v-bind:style="getBackground(r + '' + c)"
            :key="c"
            :id="r + '/' + c"
            :data-id="r + '' + c"
            v-on:click="markCell"
        ...
        <div
          class="btn-increase increase-columns"
          v-on:click="increaseCells('col')"
        >
          <p class="btn-increase-text">Добавить столбцы</p>
        </div>
        <div
          class="btn-increase increase-rows"
          v-on:click="increaseCells('rows')"
        >
  ...
  methods: {
    ...
    markCell(event) {
      ...
      for (let el of this.doingsArray) {
        if (
          el.id === this.choseDoing &&
          this.isCellClear(event.target.style.background)
        ) {
          event.target.style.background = el.background
            ? el.background

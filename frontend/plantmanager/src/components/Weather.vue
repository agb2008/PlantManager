  ...
  <div class="h-full">
    <div class="flex items-center justify-start">
      <div class="flex flex-col bg-white rounded p-4 w-full max-w-xs">
        ...
        <p class="underline text-red-500" v-if="notCityFound">К сожалению, такой город не найден!</p>
        <div v-if="visible">
          <div class="font-bold text-xl">{{ weather.cityName }}</div>
          <div class="text-sm text-gray-500">{{ weather.date }}</div>
          <div
...
export default {
  ...
  methods: {
    getWeather: async function() {
      ...
      let key = "dcd5629b25f777681467ef325df6256c";
      let url = `http://api.openweathermap.org/data/2.5/find?q=${this.citySearch}&appid=${key}&units=metric`;
